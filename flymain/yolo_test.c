...
void yolo_test(char *devicename, const char *image_path, const char *out_path, int id);
...
static void usage(const char* name)
{
	...
	printf("  -%c (--%s) the result of inference ouput as bin.\n", long_opts[i].val, long_opts[i].name); i++; 
...
int main(int argc, char* argv[])
{
	...
	/*printf("Begin to inference for image file<%s> on device file<%s>\n"
	...
	//printf("Get User BAR access addr = 0x%llx\n", userbar_addr);
	bypassbar_addr = (unsigned int *)_mmap4bar(BYPASS_BAR_PHY_ADDR, BYPASS_BAR_PHY_LEN);
	//printf("Get Bypass BAR access addr = 0x%llx\n", bypassbar_addr);
	...
	if (cfg_flp) {
		if (yolo_cfg_load(device, cfg_flp, id) < 0)
			goto FAILED_EXIT;
		//printf("[Linc]>>>>>> Trigger start at 0x%lx, wait for finish at 0x%lx\n", FPGA_690T_START_REG(id), FPGA_690T_FINISH_REG(id));
		fpga_trigger((uint32_t *)FPGA_690T_START_REG(id), (uint32_t *)FPGA_690T_FINISH_REG(id), 0);
		//printf("Loading Configuration on 690T-%d Done!\n", id);
	}
	/* Running image inference on 690t */
	if (image_flp) {
		yolo_test(device, image_flp, res_flp, id);
		//printf("Test on 690T-%d Done!\n", id);
	}
FAILED_EXIT:
...
static int fpga_690t_load(char *devicename, const char *filename, int id)
{
	...
	//printf("Downloading FPGA bin file<%s>[size=0x%lx] on 690T-%d.\n", filename, size, id);
	...
	fclose(bin_fd);
	if (rc != size) {
		//printf("ERROR: read data size error, rc=%d.\n", rc);
		free(allocated);
		return -1;
	}
	...
	barrier();
	//printf("Downloading 690T-%d Done!\n", id);
	return rc;
}
...
static int yolo_cfg_load(char *devicename, const char *filename, int id)
{
	...
	//printf("Downloading Config data file<%s>[size=0x%lx] on 690T.\n", filename, size);
	posix_memalign((void **)&allocated, 4096/*alignment*/, size + 4096);	
	assert(allocated);
	buffer = allocated;
	memset(buffer , 0, size);
	...
	//printf("Downloading Config Data Done!\n");
	return rc;
}
...
void yolo_test(char *devicename, const char *image_path, const char *out_path, int id)
{
	...
	//printf("Downloading Image data file<%s>[size=0x%lx] on 690T.\n", image_path, size);
	posix_memalign((void **)&allocated, 4096/*alignment*/, size + 4096);	
	assert(allocated);
	buffer = allocated;
	memset(buffer , 0, size);
	...
	barrier();
	//printf("Downloading Image Data Done!\n");
	//fpga_trigger((uint32_t *)FPGA_690T_START_REG(id), (uint32_t *)FPGA_690T_FINISH_REG(id), 0);
	fly_reg_wr(1, FPGA_690T_START_REG(id));
	barrier();
	clock_gettime(CLOCK_MONOTONIC, &ts_start);
	dect = 0;
	do {
	...
	clock_gettime(CLOCK_MONOTONIC, &ts_end);
        /* subtract the start time from the end time */
	timespec_sub(&ts_end, &ts_start);
	fprintf(stderr, "Image inference Done--->Spend %ld.%09ld seconds (total).\n", ts_end.tv_sec, ts_end.tv_nsec);
	fly_reg_wr(0, FPGA_690T_REG(id, 0x18));

	obj_num = fly_reg_rd(FPGA_690T_REG(id, 0x1C));	
	barrier();
	//printf("Detect %d objects in this image. saving result? <%s>\n",
	...
	/*for (i = 0; i < obj_num; i++) {
		printf("=========  Object-<%d> ============\n", i);
		printf("Object Class is %d\n", obj[i].cls);
		printf("Object Probability is %f\n", obj[i].prob);
		printf("Box position is X-(%d, %d),  Y-(%d. %d)\n", obj[i].box_x0, obj[i].box_x1, obj[i].box_y0, obj[i].box_y1);
		printf("===================================\n");
	}*/
	if (obj_num) {
